...
You probably won't need it very often, but it can be useful if the finder isn't quite returning the results you expected. Read more about the `dumpSimple` method in the [Отладка переменной](/documentation/DevelopmentTools.md#part20) section.
...
Even though for a lot of entity types a finder class doesn't exist, it is still possible to extend these non existent classes in the same way as mentioned in the [Расширение классов](/documentation/GeneralConcepts.md#part6) section.
...
This relation is able to return the records from the xf_user_connected_account table that match the current user ID as a `FinderCollection`. This is similar to the `ArrayCollection` object we mentioned in The [Поисковики](/documentation/EntitiesFindersAndRepositories.md#part1) section above. The relation definition specifies that the collection should be keyed by the `provider` field.
...
Entity options are a way of modifying the behavior of the entity under certain conditions. For example, if we set `admin_edit` to true (which is the case when editing a user in the ACP), then certain checks will be skipped such as to allow a user's email address to be empty.
